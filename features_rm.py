#MODELLI DI PREVISIONE NN E RF CON MRI E CT SEPARATI, PREVISIONE A 2 STATI
...
# select the rows from openC relative to patients with MRI patients
...
# compute the 'alive after array' Y for MRI patients
Y_mri = utl.compute_alive_after(df_openC_mri,start='clinical_Date_of_first_Diagnosis',
...
Y_ct = utl.compute_alive_after(df_openC_ct,start='clinical_Date_of_first_Diagnosis',
end='follow_Date_of_Death',alive_after=5*365)
...
model_mri, results_mri, history_mri= utl.rm_mri_approach(X_mri,Y_mri,mri_features)

# train the neural network model on ct images patients
model_ct, results_ct, history_ct = utl.rm_ct_approach(X_ct,Y_ct,ct_features)
